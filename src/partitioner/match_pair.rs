...
mod test {
    ...
    fn maybe_match_pair_test() {
        ...
        let from_file2_1 = LogEntry::from_str("3,e,f,filename2_1", File2).expect("");
        let from_file2_2 = LogEntry::from_str("4,g,h,filename2_2", File2).expect("");
        ...
        assert_eq!(MatchPair::maybe_match_pair(&from_file2_1, &from_file2_1), None);
        assert_eq!(MatchPair::maybe_match_pair(&from_file2_1, &from_file2_2), None);
        assert_eq!(MatchPair::maybe_match_pair(&from_file2_2, &from_file2_1), None);

        assert_eq!(MatchPair::maybe_match_pair(&from_file1_1, &from_file2_1),
                   Some(MatchPair{from_file1: &from_file1_1, from_file2: &from_file2_1, _prevent_struct_literal: ()}));

        assert_eq!(MatchPair::maybe_match_pair(&from_file2_2, &from_file1_2),
